...
pgdir_walk(pde_t *pgdir, const void *va, int create) {
    ...
    pde_t *pdr_entry_idx = pgdir + PDX(va);
    if (!(*pdr_entry_idx & PTE_P)) {
        if (!create)
            return NULL;
        else {
            if ((new_page = page_alloc(ALLOC_ZERO)) == NULL)
                return NULL;
            new_page->pp_ref++;
            *pdr_entry_idx = (page2pa(new_page) | PTE_P | PTE_W | PTE_U);
        }
    }

    pte_t *page_base = KADDR(PTE_ADDR(*pdr_entry_idx));
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm) {
    // Fill this function in
    pte_t *entry = NULL;
    if ((entry = pgdir_walk(pgdir, va, 1)) == NULL)
        return -E_NO_MEM;
